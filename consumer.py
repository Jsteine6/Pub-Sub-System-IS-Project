...
#GCP variables
project_id = "propane-bearing-485422-u7"
...
# Check if payload is missing any fields and accurate data types
...
def insert_message(payload: dict, is_duplicate: bool):
    cursor.execute("""
        INSERT INTO messages (message_id, content, created_at, is_duplicate)
        VALUES (?, ?, ?, ?)
    """, (
        payload["message_id"],
        payload["content"],
...
def callback(message: pubsub_v1.subscriber.message.Message) -> None:
    try:
        ...
        # insert message to DB and add duplicate flag
        insert_message(payload, is_duplicate)
        print("Message added to database: ", payload, is_duplicate)
        ...
        # ack invalid messages to infinite loop
        message.ack()
